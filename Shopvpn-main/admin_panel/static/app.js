...
/* ============================================================== api === */
// در Railway پنل زیر /admin mount می‌شود. اجرای مستقل قبلی در ریشه هم بدون
// تغییر کار می‌کند، چون در آن حالت این مقدار خالی است.
const ADMIN_BASE_PATH = location.pathname === '/admin' || location.pathname.startsWith('/admin/') ? '/admin' : '';
const adminUrl = path => ADMIN_BASE_PATH + path;

async function api(path, opts = {}) {
  const res = await fetch(adminUrl('/api' + path), {
...
function showReceiptModal(kind, id) {
  ...
  const url = adminUrl(`/api/${paths[kind] || `orders/${id}`}/receipt`);
...
async function boot_setup() {
  ...
  try {
    const info = await fetch(adminUrl(`/api/setup/info?b=${encodeURIComponent(b)}&t=${encodeURIComponent(t)}`)).then(async r => {
...
async function enablePushNotifications() {
  ...
  try {
    ...
    const reg = await navigator.serviceWorker.register(adminUrl('/sw.js'));
    ...
    <div class="card" data-banner-row="${i}" style="margin-bottom:10px">
      ...
      <label class="field"><span>آدرس تصویر (اختیاری)</span>
        <div style="display:flex;gap:8px;align-items:center">
          <input class="input" data-b-image value="${esc(b.image_url || '')}" style="direction:ltr;text-align:left" placeholder="uploads/banners/...">
  ...
  const draw = (list) => {
    ...
    $$('#banners-list [data-banner-row]', root).forEach((row) => {
      ...
      fileInput.addEventListener('change', async () => {
        ...
        try {
          const res = await fetch(adminUrl('/api/banners/upload-image'), { method: 'POST', credentials: 'include', body: fd });
  ...
  function renderRestoreConfirmStep() {
    ...
    $('#restore-final-btn', area).addEventListener('click', async () => {
      ...
      try {
        ...
        const res = await fetch(adminUrl('/api/system/backup/restore'), { method: 'POST', credentials: 'include', body: formData });
        const data = await res.json().catch(() => ({}));
