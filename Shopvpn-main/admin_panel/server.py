...
from config import DB_PATH, BOT_TOKEN, OWNER_ID, ADMIN_PANEL_SECRET, VAPID_PUBLIC_KEY, resolve_db_path, API_BASE_URL, DATA_DIR
...
async def api_upload_banner_image(photo: UploadFile = File(...), admin=Depends(require_permission("settings"))):
    ...
    # فایل آپلودی نباید داخل سورسِ موقتی کانتینر Railway ذخیره شود.
    dest_dir = os.path.join(DATA_DIR, "uploads", "banners")
    os.makedirs(dest_dir, exist_ok=True)
    with open(os.path.join(dest_dir, fname), "wb") as f:
        f.write(content)
    # آدرس نسبی، هم در اجرای مستقل و هم هنگام mount شدن پنل زیر /admin درست است.
    return {"url": f"uploads/banners/{fname}"}
...
def serve_manifest(request: Request):
    ...
    # request.scope["root_path"] هنگام اجرای Railway برابر /admin است.
    prefix = request.scope.get("root_path", "").rstrip("/")
    start_url = f"{prefix}/?b={b_value}&source=pwa" if b_value else f"{prefix}/?source=pwa"
    manifest = {
        ...
        "lang": "fa",
        "icons": [
            {"src": f"{prefix}/assets/icons/icon-192.png", "sizes": "192x192", "type": "image/png", "purpose": "any"},
            {"src": f"{prefix}/assets/icons/icon-512.png", "sizes": "512x512", "type": "image/png", "purpose": "any"},
            {"src": f"{prefix}/assets/icons/icon-maskable-192.png", "sizes": "192x192", "type": "image/png", "purpose": "maskable"},
            {"src": f"{prefix}/assets/icons/icon-maskable-512.png", "sizes": "512x512", "type": "image/png", "purpose": "maskable"},
        ],
    }
